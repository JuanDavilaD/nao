...
from lysmart_core.commands.sync.providers.databases.provider import sync_database
from lysmart_core.config.databases.base import DatabaseAccessor
...
def run_sync_with_mocks(db_config, engine, tmp_path, progress):
    ...
    with patch("lysmart_core.commands.sync.providers.databases.provider.console") as mock_console:
        with patch(
            "lysmart_core.commands.sync.providers.databases.provider.get_template_engine",
            return_value=engine,
        ):
