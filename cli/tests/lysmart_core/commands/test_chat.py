...
from lysmart_core.commands.chat import (
    chat,
    ensure_auth_secret,
...
from lysmart_core.config.base import LySmartConfig, LySmartConfigError
...
def test_try_load_returns_none_when_file_not_found(tmp_path: Path):
    cfg = LySmartConfig.try_load(tmp_path)
    assert cfg is None


def test_try_load_returns_none_when_invalid_yaml(tmp_path: Path):
    invalid_yaml = tmp_path / "lysmart_config.yaml"
...
def test_try_load_returns_none_when_validation_error(tmp_path: Path):
    invalid_config = tmp_path / "lysmart_config.yaml"
...
def test_try_load_returns_config_when_valid(tmp_path: Path):
    valid_yaml = tmp_path / "lysmart_config.yaml"
    valid_yaml.write_text("project_name: test-project")

    cfg = LySmartConfig.try_load(tmp_path)

    assert cfg is not None
    assert isinstance(cfg, LySmartConfig)
...
def test_try_load_exits_on_file_not_found(tmp_path: Path):
    with patch("lysmart_core.config.base.Console") as mock_console_cls:
        mock_console = mock_console_cls.return_value
        with pytest.raises(SystemExit) as exc_info:
            LySmartConfig.try_load(tmp_path, exit_on_error=True)

        assert exc_info.value.code == 1
        mock_console.print.assert_any_call("[bold red]✗[/bold red] No lysmart_config.yaml found in current directory")
...
def test_try_load_exits_on_invalid_yaml(tmp_path: Path):
    ...
    with patch("lysmart_core.config.base.Console") as mock_console_cls:
        ...
        assert "Failed to load lysmart_config.yaml" in call_args
...
def test_try_load_raises_on_file_not_found(tmp_path: Path):
    with pytest.raises(LySmartConfigError) as exc_info:
        LySmartConfig.try_load(tmp_path, raise_on_error=True)

    assert "No lysmart_config.yaml found" in str(exc_info.value)
...
def test_try_load_raises_on_validation_error(tmp_path: Path):
    ...
    assert "Failed to load lysmart_config.yaml" in str(exc_info.value)
...
def test_chat_exits_when_no_config_found(tmp_path: Path, monkeypatch, clean_env):
    ...
    with patch("lysmart_core.config.base.Console"):
        with pytest.raises(SystemExit) as exc_info:
            chat()
...
def test_get_server_binary_path():
    ...
    assert result_path.name == "lysmart-chat-server"
...
def test_get_server_binary_path_does_not_exists():
    ...
    with patch("lysmart_core.commands.chat.Path.exists", return_value=False):
        with pytest.raises(SystemExit) as exc_info:
            get_server_binary_path()
...
class TestEnsureAuthSecret:
    def test_returns_none_when_env_var_already_set(self, tmp_path: Path, monkeypatch):
        ...
        with patch("lysmart_core.commands.chat.console"):
    ...
    def test_loads_existing_secret_from_file(self, tmp_path: Path):
        ...
        secret_file = tmp_path / ".lysmart-secret"
...
class TestChatCommand:
    ...
    @patch("lysmart_core.commands.chat.wait_for_server")
    @patch("lysmart_core.commands.chat.subprocess.Popen")
    @patch("lysmart_core.commands.chat.get_fastapi_main_path")
    @patch("lysmart_core.commands.chat.get_server_binary_path")
    @patch("lysmart_core.commands.chat.console")
    def test_chat_starts_both_servers(
        ...
        mock_binary_path.return_value = bin_dir / "lysmart-chat-server"
...
""")
        ...
        bin_dir.mkdir()
        (bin_dir / "lysmart-chat-server").touch()
