...
from lysmart_core.commands.debug import check_llm_connection, debug
from lysmart_core.config.databases import BigQueryConfig, DuckDBConfig, PostgresConfig, TrinoConfig
from lysmart_core.config.llm import LLMConfig, LLMProvider
...
class TestDatabaseConnection:
    ...
    def test_trino_get_schemas_filters_builtins_and_nullish_values(self):
        config = TrinoConfig(name="test", host="localhost", port=8080, catalog="hive", user="lysmart")
        mock_conn = MagicMock()
...
class TestDebugCommand:
    ...
    def test_exits_when_no_config_found(self, tmp_path, monkeypatch):
        ...
        with patch("lysmart_core.commands.debug.console"):
...
""")
        ...
        with patch(
            "lysmart_core.config.databases.postgres.PostgresConfig.check_connection",
            return_value=(True, "Connected (5 tables found)"),
        ):
            with patch("lysmart_core.commands.debug.console") as mock_console:
                debug()
...
""")
        ...
        with patch(
            "lysmart_core.commands.debug.check_llm_connection",
...
""")
        ...
        with patch(
            "lysmart_core.commands.debug.check_llm_connection", return_value=(False, "API key is not working")
        ) as mock_check:
            with patch("lysmart_core.commands.debug.console") as mock_console:
                debug()
