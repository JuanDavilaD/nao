"""Check for newer lysmart-core versions on PyPI."""
...
from lysmart_core.ui import UI

CACHE_FILE = Path.home() / ".lysmart" / "version_check.json"
PYPI_URL = "https://pypi.org/pypi/lysmart-core/json"
...
def check_for_updates() -> None:
    """Check PyPI for a newer version of lysmart-core, using a 24h local cache."""
    try:
        ...
        if parse_version(latest) > parse_version(__version__):
            UI.warn(f"Update available: {__version__} → {latest}. Run: lysmart upgrade")
    except Exception:
        pass  # do nothing
