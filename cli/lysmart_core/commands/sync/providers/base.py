...
class SyncProvider(ABC):
    ...
    (e.g., repositories, databases) from the LySmart configuration to local files.
    """
    ...
    @abstractmethod
    def get_items(self, config: LySmartConfig) -> list[Any]:
    ...
    def sync(self, items: list[Any], output_path: Path, project_path: Path | None = None) -> SyncResult:
        ...
        Args:
                ...
                project_path: Path to the LySmart project root (for template resolution)
    ...
    def should_sync(self, config: LySmartConfig) -> bool:
    ...
    def pre_sync(self, config: LySmartConfig, output_path: Path) -> None:
        ...
        Args:
            config: The loaded LySmart configuration.
