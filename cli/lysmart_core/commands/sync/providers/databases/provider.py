...
from lysmart_core.commands.sync.cleanup import DatabaseSyncState, cleanup_stale_databases, cleanup_stale_paths
from lysmart_core.config import AnyDatabaseConfig, LySmartConfig
from lysmart_core.config.databases.base import DatabaseConfig
from lysmart_core.templates.engine import get_template_engine
...
class DatabaseSyncProvider(SyncProvider):
    ...
    def pre_sync(self, config: LySmartConfig, output_path: Path) -> None:
        cleanup_stale_databases(config.databases, output_path, verbose=True)

    def get_items(self, config: LySmartConfig) -> list[AnyDatabaseConfig]:
