...
from lysmart_core.templates.render import render_all_templates
...
def sync(
    *,
    ...
    the project directory, making the `lysmart` context object available for
    accessing provider data.
    """
    console.print("\n[bold cyan]🔄 LySmart sync[/bold cyan]\n")

    config = LySmartConfig.try_load(exit_on_error=True)
    assert config is not None  # Help type checker after exit_on_error=True

    # Get project path (current working directory after LySmartConfig.try_load)
