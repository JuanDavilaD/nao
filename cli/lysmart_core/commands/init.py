...
from lysmart_core.tracking import track_command
from lysmart_core.ui import UI, ask_confirm, ask_text
...
def setup_project_name(force: bool = False) -> tuple[str, Path, LySmartConfig | None]:
    """Setup the project name. Returns existing config if found and user wants to extend."""
    # Check if we're in a directory with an existing lysmart_config.yaml
    current_dir = Path.cwd()
    config_file = current_dir / "lysmart_config.yaml"

    if config_file.exists():
        # Load existing config to get project name
        existing_config = LySmartConfig.try_load(current_dir)
        if existing_config:
            UI.title("Found existing lysmart_config.yaml")
...
def create_empty_structure(project_path: Path) -> tuple[list[str], list[CreatedFile]]:
    ...
    FILES = [
        ...
        CreatedFile(path=Path(".lysmart_ignore"), content="templates/\n*.j2\n"),
    ]
...
def init(
    *,
    ...
    Creates a project folder with a lysmart_config.yaml configuration file.
    ...
    """
    UI.info("\n🚀 LySmart project initialization\n")

    try:
        project_name, project_path, existing_config = setup_project_name(force=force)
        config = LySmartConfig.promptConfig(project_name, existing=existing_config)
        ...
        UI.success(f"Saved [dim]{project_path / 'lysmart_config.yaml'}[/dim]")
        UI.print()
        UI.print("[bold green]Done![/bold green] Your LySmart project is ready. 🎉")
        ...
        if has_connections:
            ...
            from lysmart_core.commands.debug import debug
...
[cyan]lysmart debug[/cyan]   - Test connectivity to your configured databases and LLM
              Verifies that all connections are working properly

[cyan]lysmart sync[/cyan]    - Sync database schemas to local markdown files
              Creates documentation for your tables and columns

[cyan]LySmart chat[/cyan]    - Start the LySmart chat interface
