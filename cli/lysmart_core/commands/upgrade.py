...
from lysmart_core.ui import UI, ask_confirm
from lysmart_core.version import clear_version_cache, get_latest_version, parse_version


def upgrade() -> None:
    """Upgrade lysmart-core to the latest version."""
    ...
    # Perform upgrade
    UI.print(f"\n🔄 Upgrading lysmart-core {current_version} → {latest_version}...\n")

    # Use uv if available, otherwise fallback to pip
    if shutil.which("uv"):
        cmd = ["uv", "pip", "install", "--upgrade", "lysmart-core"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "lysmart-core"]
    ...
    try:
        ...
        UI.print("  pip install --upgrade lysmart-core")
