...
from lysmart_core import __version__
from lysmart_core.config import LySmartConfig
from lysmart_core.config.llm import PROVIDER_AUTH, LLMProvider
from lysmart_core.mode import MODE
from lysmart_core.tracking import track_command

console = Console()

# Default port for the LySmart chat server
DEFAULT_SERVER_PORT = 5005
FASTAPI_PORT = 8005
SECRET_FILE_NAME = ".lysmart-secret"
...
def get_server_binary_path() -> Path:
    """Get the path to the bundled lysmart-chat-server binary."""
    # The binary is in the bin folder relative to this file
    cli_dir = Path(__file__).parent.parent
    bin_dir = cli_dir / "bin"
    binary_path = bin_dir / "lysmart-chat-server"
...
def chat(port: Annotated[Optional[int], Parameter(name=["-p", "--port"])] = None):
    ...
    Launches the LySmart chat server and opens the web interface in your browser.
    ...
    """
    console.print("\n[bold cyan]💬 Starting LySmart chat...[/bold cyan]\n")

    # Try to load LySmart config from current directory
    config = LySmartConfig.try_load(exit_on_error=True)
    assert config is not None  # Help type checker after exit_on_error=True
    console.print(f"[bold green]✓[/bold green] Loaded config from {Path.cwd() / 'lysmart_config.yaml'}")
