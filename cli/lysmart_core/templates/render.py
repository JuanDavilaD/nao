...
This module discovers and renders `.j2` files in the LySmart context folder,
making the `lysmart` object available for accessing provider data.
...
from .context import create_lysmart_context
...
def discover_templates(
    ...
    Args:
        project_path: Path to the LySmart project root.
...
def render_template(
    ...
    # Create the LySmart context
    lysmart_ctx = create_lysmart_context(config)

    # Load and render the template
    template = env.get_template(str(template_path))
    rendered = template.render(lysmart=lysmart_ctx)
