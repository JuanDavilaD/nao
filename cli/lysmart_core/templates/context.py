"""Context object for Jinja templates in the LySmart context folder.

This module provides the `lysmart` object that is exposed to user Jinja templates,
...
Example template usage:
    ...
    {{ lysmart.notion.page('abc123').title }}
"""
...
if TYPE_CHECKING:
    from lysmart_core.config.base import LySmartConfig
...
class NotionPage:
    ...
    def _load(self) -> dict[str, Any]:
        ...
        if self._data is None:
            ...
            from lysmart_core.commands.sync.providers.notion.provider import (
                extract_page_id,
                get_page_title,
                strip_images,
            )
...
class NotionProvider:
    ...
    def __init__(self, config: LySmartConfig):
        self._config = config
    ...
    def _get_api_key_for_page(self, page_url_or_id: str) -> str:
        ...
        """
        from lysmart_core.commands.sync.providers.notion.provider import extract_page_id
    ...
    def page(self, page_url_or_id: str) -> NotionPage:
        ...
        Example:
            {{ lysmart.notion.page('https://notion.so/My-Page-abc123').content }}
            {{ lysmart.notion.page('abc123def456...').title }}
        """
...
class LySmartContext:
    """The main context object exposed as `lysmart` in user templates.
    ...
    Example template usage:
        {{ lysmart.notion.page('url').content }}
    ...
    @property
    def config(self) -> LySmartConfig:
        """Access the LySmart configuration.
...
def create_lysmart_context(config: LySmartConfig) -> LySmartContext:
    """Create a LySmartContext for template rendering.

    Args:
        config: The LySmart configuration.

    Returns:
        A LySmartContext instance to be used as `lysmart` in templates.
    """
    return LySmartContext(config)
