...
from lysmart_core.ui import UI, ask_confirm, ask_select
...
class LySmartConfigError(Exception):
    """Raised when LySmart config loading fails."""

    pass


class LySmartConfig(BaseModel):
    """LySmart project configuration."""

    project_name: str = Field(description="The name of the LySmart project")
    ...
    @classmethod
    def promptConfig(cls, project_name: str, existing: "LySmartConfig | None" = None) -> "LySmartConfig":
        """Interactively prompt the user for all LySmart configuration options.
    ...
    @classmethod
    def _prompt_extend(cls, existing: "LySmartConfig") -> "LySmartConfig":
    ...
    @classmethod
    def load(cls, path: Path) -> "LySmartConfig":
    ...
    def try_load(
        ...
        *,
        ...
        def handle_error(message: str) -> None:
            if raise_on_error:
                raise LySmartConfigError(message)
            if exit_on_error:
                console = Console()
        ...
        if not config_file.exists():
            handle_error("No lysmart_config.yaml found in current directory")
            return None
        ...
        try:
            ...
            handle_error(f"Failed to load lysmart_config.yaml: Invalid YAML syntax: {e}")
            return None
        except ValidationError as e:
            errors = "; ".join(
                f"{' → '.join(str(x) for x in err['loc']) or 'config'}: {err['msg']}" for err in e.errors()
            )
            handle_error(f"Failed to load lysmart_config.yaml: {errors}")
            return None
        except ValueError as e:
            handle_error(f"Failed to load lysmart_config.yaml: {e}")
            return None
